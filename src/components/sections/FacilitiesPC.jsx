...
export default function FacilitiesPC() {
  ...
  // ★ 無限ループの幅を JS で計算して CSS 変数に渡す
  useEffect(() => {
    const track = trackRef.current;
    if (!track) return;

    const totalWidth = track.scrollWidth / 2; // 1セットぶんの幅
  ...
  return (
    <section
      ref={sectionRef}
      className="hidden md:block w-full py-[200px] bg-white overflow-hidden"
    >
      <div className="w-[90%] max-w-[1400px] mx-auto">
        ...
        <div
          ref={trackRef}
          className={`
            mt-20 pb-6 flex gap-14 whitespace-nowrap
        ...
        >
          {items.map((f, i) => (
            <div key={`${f.id}-${i}`} className="min-w-[520px] inline-block">
              <div className="aspect-[4/3] overflow-hidden rounded-[6px] bg-neutral-200 relative group">
                <img
                  src={f.img}
                  alt={f.title}
              ...
              <h3 className="text-[22px] font-light mt-5 text-neutral-900">{f.title}</h3>
              <p className="text-[15px] text-neutral-600 mt-2 leading-[1.8]">{f.text}</p>
            </div>
          ))}
        </div>
      </div>
